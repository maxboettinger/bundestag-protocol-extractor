...
class PatternExtractionStrategy(ExtractionStrategy):
    ...
    PAGE_MARKER = r'Deutscher Bundestag[\s–-]+\d+\.\s+Wahlperiode[\s–-]+(\d+)\.\s+Sitzung\.'
    INTERJECTION_START = r'\((?:Beifall|\s*Zuruf|Lachen|Heiterkeit|Widerspruch)'
    INTERJECTION_FULL = r'\([^)]{2,100}\)'
    SPEECH_END_MARKER = r'(?:Vielen|Herzlichen)?\s*Dank(?:\s*für\s*[Ii]hre\s*Aufmerksamkeit)?\.?'
    ...
    def extract(self, protocol: PlenarProtocol, speeches: List[Speech]) -> List[Speech]:
        ...
        """
        logger.info(f"Extracting speeches using pattern strategy for protocol {protocol.dokumentnummer}")
        ...
        for speech in speeches:
            ...
            if speech.page_start and speech.page_start in page_indices:
                ...
                speaker_idx = self._find_speaker_near_page(
                    full_text, speaker_matches, speaker_name, speaker_last_name, start_idx
                )
                ...
                if speaker_idx != -1:
                    ...
                    if speech_text:
                        ...
                        if not is_partial:
                            ...
                            interjection_count = len(re.findall(self.INTERJECTION_FULL, speech_text))
                            interjection_factor = min(interjection_count / 5, 1.0) * 0.1
                            
                            # Apply the confidence adjustments
                            speech.extraction_confidence += length_factor + interjection_factor
                            
                            # Cap at base confidence
                            speech.extraction_confidence = min(speech.extraction_confidence, self.base_confidence)
            ...
            if speaker_idx != -1:
                ...
                if speech_text:
                    ...
                    speech.extraction_method = metadata["extraction_method"]
                    speech.extraction_status = metadata["extraction_status"]
                    speech.extraction_confidence = metadata["extraction_confidence"] * 0.9  # Slightly lower confidence when not using page
                    continue
    ...
    def _find_speaker_near_page(
        self, text: str, speaker_matches: List[Match], 
        speaker_name: str, speaker_last_name: str, 
        page_start_idx: int
    ) -> int:
        """
    ...
    def _extract_speech_content(
        self, text: str, speaker_idx: int, 
        speaker_matches: List[Match], speaker_name: str
    ) -> Tuple[str, bool]:
        """
