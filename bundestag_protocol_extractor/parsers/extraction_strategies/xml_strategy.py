...
from bundestag_protocol_extractor.models.schema import Speech, PlenarProtocol, Person
...
class XMLExtractionStrategy(ExtractionStrategy):
    ...
    def extract(self, protocol: PlenarProtocol, speeches: List[Speech]) -> List[Speech]:
        ...
        """
        logger.info(f"Extracting speeches using XML strategy for protocol {protocol.dokumentnummer}")
        ...
        if xml_root is None:
            logger.warning(f"XML extraction failed for protocol {protocol.dokumentnummer}")
        ...
        for speech in speeches:
            ...
            if speech.page_start and speech.page_start in page_speeches:
                ...
                for raw_speech in page_speeches[speech.page_start]:
                    ...
                    if speaker_last_name and raw_last_name and speaker_last_name in raw_last_name or raw_last_name in speaker_last_name:
                    ...
                    if page_speeches[speech.page_start]:
                        self._apply_xml_speech_data(speech, page_speeches[speech.page_start][0])
                    else:
    ...
    def _apply_xml_speech_data(self, speech: Speech, raw_speech: Dict[str, Any]) -> None:
        """
        ...
        speech.is_presidential_announcement = raw_speech.get("is_presidential_announcement", False)