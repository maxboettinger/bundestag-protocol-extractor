...
from bundestag_protocol_extractor.models.schema import Speech, PlenarProtocol
...
class DataQualityReporter:
    ...
    def generate_quality_report(
        ...
        method_percentages = (df_speeches["extraction_method"].value_counts(normalize=True) * 100).to_dict()
        status_percentages = (df_speeches["extraction_status"].value_counts(normalize=True) * 100).to_dict()
        ...
        high_confidence = df_speeches[df_speeches["extraction_confidence"] >= 0.8].shape[0]
        medium_confidence = df_speeches[(df_speeches["extraction_confidence"] >= 0.5) & 
                                       (df_speeches["extraction_confidence"] < 0.8)].shape[0]
        low_confidence = df_speeches[(df_speeches["extraction_confidence"] >= 0.2) & 
                                    (df_speeches["extraction_confidence"] < 0.5)].shape[0]
        very_low_confidence = df_speeches[df_speeches["extraction_confidence"] < 0.2].shape[0]
        ...
        if "text" in df_speeches.columns:
            ...
            truncated_count = df_speeches[df_speeches["text"].str.contains("EXTRACTION_FAILED")].shape[0]
            truncated_percentage = (truncated_count / total_speeches) * 100 if total_speeches > 0 else 0
        ...
        if protocol_metadata is not None and not protocol_metadata.empty:
            ...
            protocol_stats = {
                ...
                "protocols_without_speeches": protocol_count - df_protocols_with_speeches,
        ...
        report = {
            ...
            "confidence_metrics": {
                ...
                "distribution": {
                    ...
                    "high_confidence_percentage": (high_confidence / total_speeches) * 100,
                    "medium_confidence": medium_confidence,
                    "medium_confidence_percentage": (medium_confidence / total_speeches) * 100,
                    "low_confidence": low_confidence,
                    "low_confidence_percentage": (low_confidence / total_speeches) * 100,
                    "very_low_confidence": very_low_confidence,
                    "very_low_confidence_percentage": (very_low_confidence / total_speeches) * 100
                }
            },
            "text_metrics": {
    ...
    def generate_quality_visualizations(
        ...
        ax1.pie(method_counts, labels=method_counts.index, autopct='%1.1f%%', startangle=90, 
               shadow=True, explode=[0.1 if m == "xml" else 0 for m in method_counts.index])
        ax1.axis('equal')
        ...
        ax2.pie(status_counts, labels=status_counts.index, autopct='%1.1f%%', startangle=90,
               shadow=True, explode=[0.1 if s == "complete" else 0 for s in status_counts.index])
        ax2.axis('equal')
        ...
        ax3.hist(df_speeches["extraction_confidence"], bins=20, color='skyblue', edgecolor='black')
        ax3.set_title('Distribution of Extraction Confidence Scores')
        ax3.set_xlabel('Confidence Score')
        ax3.set_ylabel('Number of Speeches')
        ax3.grid(True, linestyle='--', alpha=0.7)
        ax3.axvline(0.8, color='red', linestyle='--', label='High Confidence Threshold (0.8)')
        ax3.axvline(0.5, color='orange', linestyle='--', label='Medium Confidence Threshold (0.5)')
        ax3.axvline(0.2, color='green', linestyle='--', label='Low Confidence Threshold (0.2)')
        ax3.legend()
        
        if save_plots:
            confidence_plot_path = figures_dir / f"{base_filename}_confidence_distribution.png"
        ...
        if "text" in df_speeches.columns:
            ...
            methods = [m for m in methods if m in df_speeches["extraction_method"].unique()]
            
            # Create boxplot
            df_speeches.boxplot(column="text_length", by="extraction_method", ax=ax4, 
                               vert=True, patch_artist=True, order=methods)
            ...
            for i, method in enumerate(methods):
                method_data = df_speeches[df_speeches["extraction_method"] == method]["text_length"]
                if not method_data.empty:
                    mean_val = method_data.mean()
                    ax4.text(i+1, mean_val, f'Mean: {int(mean_val)}', 
                           horizontalalignment='center', size='small', color='black', weight='semibold')
            
            if save_plots:
                length_plot_path = figures_dir / f"{base_filename}_text_length_by_method.png"
        ...
        summary = [
            ...
            f"XML Extracted: {xml_count} ({xml_count/total*100:.1f}%)" if total > 0 else "XML Extracted: 0 (0.0%)",
            f"Complete Status: {complete_count} ({complete_count/total*100:.1f}%)" if total > 0 else "Complete Status: 0 (0.0%)",
            f"High Confidence: {high_conf_count} ({high_conf_count/total*100:.1f}%)" if total > 0 else "High Confidence: 0 (0.0%)",
        ]
...
"""
        ...
        for method, metrics in report.get("text_metrics", {}).get("length_by_method", {}).items():
            html_content += f"""
            <tr>
                <td>{method}</td>
