...
class BundestagAPIClient:
    ...
    def get_plenarprotokoll_xml(
        ...
        try:
            ...
            if cache_path and cache_path.exists():
                ...
                if cached_xml:
                    try:
                        ...
                        logger.warning("Cached XML is invalid, continuing with download attempt")
                ...
                for retry in range(max_retries + 1):
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            if 'html' in content_type.lower() and '<html' in response.text[:1000].lower():
                            ...
                            if redirect_url:
                                ...
                                if response.status_code == 200:
                                    ...
                                    content_type = response.headers.get('Content-Type', '')
                                    if 'html' in content_type.lower() and '<html' in response.text[:1000].lower():
                                        logger.debug(f"Received HTML instead of XML from redirect {redirect_url}")
                                        break  # Try next URL
                                        
                                    logger.info(f"Successfully retrieved XML from redirect {redirect_url}")
                                    xml_content = response.content.decode("utf-8", errors="replace")
                                    ...
                                    if self._validate_xml(xml_content):
                                        ...
                                        repaired_xml = self._repair_xml(xml_content)
                                        if repaired_xml:
                                            logger.info(f"Successfully repaired XML from redirect {redirect_url}")
                                            xml_content = repaired_xml
                                            successful_url = redirect_url
                                            break  # Repaired XML is valid
                        else:
            ...
            if not xml_content:
                ...
                try:
                    xml_text = self.get_plenarprotokoll_text(
                        ...
                        max_retries=max_retries, retry_delay=1.0
                    )
            ...
            logger.warning(f"Could not find valid XML for protocol {doc_identifier} after trying all sources")
            return None
    ...
    def _build_xml_urls(self, metadata: Dict[str, Any]) -> List[str]:
        ...
        if document_id and hash_value:
            urls.append(f"{self.BUNDESTAG_XML_URL}/{document_id}/{hash_value}/{doc_number}.xml")
        ...
        if document_id:
            ...
            urls.append(f"{self.BUNDESTAG_XML_URL}/{document_id}/pp{wahlperiode}{dokument_nummer}.xml")
            urls.append(f"{self.BUNDESTAG_XML_URL}/{document_id}/pp{doc_number}.xml")
            urls.append(f"{self.BUNDESTAG_XML_URL}/{document_id}/plenarprotokoll_{doc_number}.xml")
        ...
        if int(wahlperiode) < 20:
            ...
            urls.append(f"https://www.bundestag.de/parlament/plenum/abstimmung/plenarprotokolle/pp{doc_number}.xml")
            urls.append(f"https://www.bundestag.de/blob/{document_id}/data.xml") if document_id else None
        ...
        try:
            if cache_path.exists():
                ...
                if self._validate_xml(xml_content):
                    ...
                    logger.warning(f"Cached XML is invalid, will re-download: {cache_path}")
                    return None
            return None
        except Exception as e:
